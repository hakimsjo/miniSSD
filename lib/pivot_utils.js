...
var PIVOT_UTILS = (function (self, $) {
    self.createPivotTableFromUrl = function (url, request, tableDivId) {
        $.support.cors = true;
        $.post(
            url,
            ...
            function (data) {
                ...
                var dataPivot = new PivotTable(tableDivId, dataVortex, dataVortex.axisList.slice(0, dataVortex.axisList.length - 1), dataVortex.axisList.slice(dataVortex.axisList.length - 1, dataVortex.axisList.length));
                dataPivot.display();
            },
            "json"
        );
